...
func TestEnsureL4BackendService(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			bs, _, err := backendPool.EnsureL4BackendService(bsName, hcLink, tc.protocol, tc.affinityType, tc.schemeType, namespacedName, network, tc.connectionTrackingPolicy, klog.TODO())
			if err != nil {
...
func TestEnsureL4BackendServiceDoesNotDetachBackends(t *testing.T) {
	...
	for _, needConnectionTrackingPolicy := range []bool{false, true} {
		t.Run("Test that EnsureL4BackendService will not detach backends", func(t *testing.T) {
			...
			bs, _, err := backendPool.EnsureL4BackendService(bsName, hcLink, "TCP", string(v1.ServiceAffinityNone), string(cloud.SchemeInternal), namespacedName, network, noConnectionTrackingPolicy, klog.TODO())
			if err != nil {
				t.Errorf("EnsureL4BackendService failed")
			}
