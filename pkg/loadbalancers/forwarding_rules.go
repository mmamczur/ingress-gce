...
func (l4netlb *L4NetLB) ensureIPv4ForwardingRule(bsLink string) (*composite.ForwardingRule, IPAddressType, bool, error) {
	...
	if err != nil {
		...
		return nil, IPAddrUndefined, false, err
	}
	...
	if err != nil {
		return nil, IPAddrUndefined, false, fmt.Errorf("Failed to compute description for forwarding rule %s, err: %w", frName,
			err)
	}
	fr := &composite.ForwardingRule{
		Name:                frName,
	...
	if existingFwdRule != nil {
		if existingFwdRule.NetworkTier != fr.NetworkTier {
			...
			return nil, IPAddrUndefined, false, networkTierMismatchError
		}
		equal, err := Equal(existingFwdRule, fr)
		if err != nil {
			return existingFwdRule, IPAddrUndefined, false, err
		}
		if equal {
			// nothing to do
			frLogger.V(2).Info("ensureIPv4ForwardingRule: Skipping update of unchanged forwarding rule")
			return existingFwdRule, isIPManaged, false, nil
		}
		frDiff := cmp.Diff(existingFwdRule, fr)
		// If the forwarding rule pointed to a backend service which does not match the controller naming scheme,
		// that resource could be leaked. It is not being deleted here because that is a user-managed resource.
		...
		if err = l4netlb.forwardingRules.Delete(existingFwdRule.Name); err != nil {
			return nil, IPAddrUndefined, true, err
		}
	...
	if err = l4netlb.forwardingRules.Create(fr); err != nil {
		if isAddressAlreadyInUseError(err) {
			return nil, IPAddrUndefined, true, utils.NewIPConfigurationError(fr.IPAddress, addressAlreadyInUseMessageExternal)
		}
		return nil, IPAddrUndefined, true, err
	}
	...
	if createdFr == nil {
		return nil, IPAddrUndefined, true, fmt.Errorf("forwarding rule %s not found", fr.Name)
	}
	return createdFr, isIPManaged, true, err
}
