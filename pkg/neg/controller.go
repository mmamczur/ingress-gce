...
func (c *Controller) mergeVmIpNEGsPortInfo(service *apiv1.Service, name types.NamespacedName, portInfoMap negtypes.PortInfoMap, negUsage *metricscollector.NegServiceState, networkInfo *network.NetworkInfo) error {
	...
	wantsCustomNegLB := l4annotations.HasLoadBalancerClass(service, l4annotations.CustomNegLoadBalancerClass)
	needsNEGForILB := c.runL4ForILB && wantsILB
	needsNEGForNetLB := c.netLBServiceNeedsNEG(service, networkInfo)
	if !needsNEGForILB && !needsNEGForNetLB && !wantsCustomNegLB {
		return nil
	}
	...
	// "networking.gke.io/l4-regional-internal" or "networking.gke.io/custom-neg-load-balancer"
	...
	if service.Spec.LoadBalancerClass != nil &&
		...
		!l4annotations.HasLoadBalancerClass(service, l4annotations.CustomNegLoadBalancerClass) {
